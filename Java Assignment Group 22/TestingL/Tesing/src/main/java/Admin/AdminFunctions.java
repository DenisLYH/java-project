...
public class AdminFunctions {
    ...
    //GO MANAGE USER PAGE
    public void UserManagement(JFrame frame) {
        frame.dispose();
        new AdminManageUser().setVisible(true);
    }
    
    //GO TOP UP PAGE
    public void TopUpCredit(JFrame frame) {
        frame.dispose();
        new AdminTopUp().setVisible(true);
    }
    
    //GO VIEW TRANSACTION PAGE
    public void GenerateReceipt(JFrame frame) {
        frame.dispose();
        new AdminViewTransaction().setVisible(true);
    }
    ...
    public void LogOut(JFrame frame) {
        ...
        if (option == JOptionPane.YES_OPTION) {
            frame.dispose(); 
            JOptionPane.showMessageDialog(null, "Bye Bye Admin " + "?" + "!", "Logout Successful", JOptionPane.INFORMATION_MESSAGE);
        } else {
...
//    public void LogOut(JFrame frame JTextField username) {
//        int option = JOptionPane.showOptionDialog(null,"Are You Sure You Want To Logout?","Logout Confirmation", JOptionPane.YES_NO_OPTION, JOptionPane.INFORMATION_MESSAGE, null, new Object[]{"YES", "NO"}, "YES");
//        if (option == JOptionPane.YES_OPTION) {
//            frame.dispose(); 
//            JOptionPane.showMessageDialog(null, "Bye Bye Admin " + username + "!", "Logout Successful", JOptionPane.INFORMATION_MESSAGE);
//        } else {
//            JOptionPane.showMessageDialog(null, "Logout Cancel Successful.", "Cancel Successful", JOptionPane.INFORMATION_MESSAGE);
//        }
//    }
            ...
            if (username.isEmpty()){
                ...
                if (confirmation == JOptionPane.YES_OPTION) {
                    ...
                    if (users != null) {
                        ...
                        Address userAd = new Address(housenumber,streetname,residentialarea,residentialarea,city,state);
                        /*user.address.houseNumber = housenumber;
                        user.address.streetName = streetname;
                        user.address.residentialArea = residentialarea;
                        user.address.postalCode = postalcode;
                        user.address.city = city;
                        user.address.state = state;*/
                        user = new User(userID,username,password,oldRole,phonenumber,userAd);
    ...
    public void AccountTableAddBalance(JTable accountTable, DefaultTableModel model, int selectedRow, JTextField accountidField, JTextField useridField, JTextField usernameField, JTextField balanceField, JTextField valueField, JTextField searchField) {
        try {
            if (selectedRow == -1) {
                accountTable.clearSelection();
                accountidField.setText("");
            ...
            if (option == JOptionPane.YES_OPTION) {
                ...
                AdminReceipt adminReceipt = new AdminReceipt(this, false);
                adminReceipt.Receipt(transactionID, customerID, username, valueadd, transactionDate, transactionTime, accountBalance);
    ...
    public void AccountTableDeductBalance(JTable accountTable, DefaultTableModel model, int selectedRow, JTextField accountidField, JTextField useridField, JTextField usernameField, JTextField balanceField, JTextField valueField, JTextField searchField) {
        try {
            if (selectedRow == -1) {
                accountTable.clearSelection();
                accountidField.setText("");
            ...
            if (option == JOptionPane.YES_OPTION) {
                ...
                AdminReceipt adminReceipt = new AdminReceipt(this, false);
                adminReceipt.Receipt(transactionID, customerID, username, valuededuct, transactionDate, transactionTime, accountBalance); // Call Receipt on the instance
                adminReceipt.setVisible(true);
            }
    ...
    ///////////////////////////////////RECEIPT//////////////////////////////////
    //VIEW RECEIPT ROW
    public void ViewReceiptRow(JFrame transaction, JTable transactionTable, java.awt.event.MouseEvent evt) {
        ...
        if (col == 7) {
            ...
            AdminReceipt adminReceipt = new AdminReceipt(this, true);
            adminReceipt.Receipt(transactionID, customerID, username, amount, transactionDate, transactionTime, accountBalance);
            adminReceipt.setVisible(true);
            transaction.dispose();
        }
    }
    ...
    //GENERATE RECEIPT
    public void GenerateReceipt(JLabel transactionidtxt, JLabel customeridtxt, JLabel usernametxt, JLabel amounttxt, JLabel transactiondatetxt, JLabel transactiontimetxt, JLabel accountbalancetxt,
    ...
    //CLOSE RECEIPT
    public void CloseReceipt(JFrame receipt){
        AdminViewTransaction transaction = new AdminViewTransaction();
        receipt.dispose();
        transaction.setVisible(true);     
    }
    
    //OK RECEIPT
    public void OkReceipt(JFrame receipt){
        receipt.dispose();
    }
    
    public void DownloadReceiptImage(JFrame receipt, String Username, JButton closeButton, JButton downloadButton, JButton okButton, boolean isTransactionPageView) {
        try {
